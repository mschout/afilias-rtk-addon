...
import com.liberty.rtk.extension.epprtk.Organization;
import com.liberty.rtk.extension.epprtk.OxrsDomain; 
import com.liberty.rtk.extension.epprtk.PolicyDelete;
...
**/
public class OrganizationSessionExample extends SessionExample
{
    ...
    {
        SessionExample example = new OrganizationSessionExample(args);
        example.session();
    }

    public OrganizationSessionExample(String args[])
    {
        super(args);
        domain_name = nextArgument();
        //domain_name = String.valueOf(new Random().nextInt(1000)) + ".info";
        year = nextArgument(); 
        role = nextArgument();
        id = nextArgument();
    ...
    protected String getUsage()
    {
        return super.getUsage()
            + " domain_name year role organization_id";
    }
    ...
    protected void process() throws epp_Exception, IOException, epp_XMLException
    {
        ...
        createDomain();
        try{
        	updateDomainRemoveReseller();
        	updateDomainAddReseller();
        	updateDomainChgReseller();
        } catch (Exception e) {
	...
	private void createDomain() throws epp_Exception, IOException, epp_XMLException
	{
        ...
        organization.setCommand("create");

		epp_Extension[] extensions = {organization};
	...
	private void updateDomainRemoveReseller() throws epp_XMLException, epp_Exception, Exception 
    {
        System.out.println("Creating the Domain Update command - Remove Reseller");


        Organization organization = new Organization();
        organization.setId(id);
        organization.setRole (role);
        organization.setCommand("rem");
        updateDomain(organization);
    }
	
	private void updateDomainAddReseller() throws epp_XMLException, epp_Exception, Exception 
    {
        System.out.println("Creating the Domain Update command - Remove Reseller");
        ...
        organization.setId(id);
        organization.setRole (role);
        organization.setCommand("add");
        updateDomain(organization);
    }
	
	private void updateDomainChgReseller() throws epp_XMLException, epp_Exception, Exception 
    {
        System.out.println("Creating the Domain Update command - Remove Reseller");


        Organization organization = new Organization();
        organization.setId("new_"+id);
        organization.setRole (role);
        organization.setCommand("chg");
        updateDomain(organization);
    }
	
	private void updateDomain(Organization organization)throws epp_XMLException, epp_Exception, Exception
	{
