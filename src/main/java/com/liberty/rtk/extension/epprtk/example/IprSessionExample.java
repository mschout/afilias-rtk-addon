...
/**
 * Example code for Liberty's Domain Ipr add-on to the RTK.
 * Uses domain create, info and update to demonstrate its usage.
 * The Domain Ipr is only required during the Sunrise period
...
**/
public class IprSessionExample extends SessionExample
{
     ...
     * the usage of the DomainIpr class in those commands.
     **/

    public static void main(String args[])
    {
        SessionExample example = new IprSessionExample(args);
        example.session();
    }

    public IprSessionExample(String args[])
    {
        super(args);
	...
	private void createDomain() throws epp_Exception, IOException, epp_XMLException
	{
		...
		IprData data = new IprData();
                ...
                data.setAppDate("2001-01-01");
                data.setRegDate("2002-01-01");
                data.setIprClass("1");
	...
	private void domainInfo() throws epp_XMLException, epp_Exception
	{
		...
		System.out.println("Ipr extension:");
		epp_Response response = domain_info_response.getRsp();
		String[] extensionStrings = response.getExtensionStrings();
		if ( extensionStrings != null ) {
			Ipr ipr = new Ipr();
			ipr.fromXML(extensionStrings[0]);
			System.out.println("Ipr Data ["+ ipr.getIprData() +"]");
		} else {
