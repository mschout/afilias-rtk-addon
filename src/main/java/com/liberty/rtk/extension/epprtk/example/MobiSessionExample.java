...
/*
 * $Header: /cvsroot/epp-rtk/liberty-rtk-addon/java/src/com/liberty/rtk/extension/epprtk/example/MobiSessionExample.java,v 1.1 2006/03/03 16:34:34 ewang2004 Exp $
 * $Revision: 1.1 $
 * $Date: 2006/03/03 16:34:34 $
 */
...
import com.liberty.rtk.extension.epprtk.MobiDomain;

/**
 * Example code for a typical logical EPP sessions to demonstrate 
 * usage of com.liberty.rtk.extension.epprtk.MobiDomain.
 *
 * @author Eric Wang
 * @version $Revision: 1.1 $ $Date: 2006/03/03 16:34:34 $
**/
public class MobiSessionExample extends SessionExample
{
    private String domain_name = null;
	private String contact_id1 = epp_client_id + "001";
	private String contact_id2 = epp_client_id + "002";

	public MobiSessionExample(String args[])
	{
        super(args);
    ...
    {
        SessionExample example = new MobiSessionExample(args);
        example.session();
    }
	...
	protected void process() throws epp_Exception, IOException, epp_XMLException
    {
        if (!checkDomain())
        {
			System.out.println("domain " + domain_name + " exists in .mobi registry, please choose another domain");
			System.exit(1);
        }
	...
	private void createDomain() throws epp_Exception, IOException, epp_XMLException
	{
		...
		MobiDomain mobi_domain_extension = new MobiDomain();
		mobi_domain_extension.setCommand("create");
		mobi_domain_extension.setMaintainerUrl("http://afilias.info");

		epp_Extension[] extensions = {mobi_domain_extension};
	...
	private void domainInfo() throws epp_XMLException, epp_Exception
	{
		...
		if ( extensionStrings != null ) {
			MobiDomain mobi_domain_extension = new MobiDomain();
			mobi_domain_extension.fromXML(extensionStrings[0]);
			System.out.println("Maintainer Url ["+mobi_domain_extension.getMaintainerUrl()+"]");
		} else {
	...
	private void domainUpdate() throws epp_XMLException, epp_Exception
    {
        ...
        MobiDomain mobi_domain_extension = new MobiDomain();
        mobi_domain_extension.setCommand("update");
        mobi_domain_extension.setMaintainerUrl("dummy@afilias.info");
        domain_update_request.getCmd().setExtension(mobi_domain_extension);
